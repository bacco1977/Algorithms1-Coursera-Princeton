...
public class KdTree {
    ...
    private Node insert(Node cur, Node toPut, boolean compX, double xmin, double ymin, double xmax,
                        double ymax) {
        if (cur == null) {
        ...
        while (!s.empty()) {
            ...
            if (cur != null) {
                ...
                if (cur.rect.distanceSquaredTo(p) < smallestDistance) {
                    testDistA = (cur.left == null) ? Integer.MAX_VALUE :
                                cur.left.rect.distanceSquaredTo(p);
                    testDistB = (cur.right == null) ? Integer.MAX_VALUE :
                                cur.right.rect.distanceSquaredTo(p);

                    if (testDistA < testDistB) {
                        if (testDistB < smallestDistance) s.push(cur.right);
                        if (testDistA < smallestDistance)
                            s.push(cur.left);       // Left path taken first (stack)
                    }
                    else {
